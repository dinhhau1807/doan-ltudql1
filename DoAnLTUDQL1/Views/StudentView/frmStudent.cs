...
namespace DoAnLTUDQL1.Views.StudentView
{
	public partial class frmStudent : MetroForm, IStudentView
	{
        ...
        #region RP In Bang Diem
        private void mbtnExamRS_Click(object sender, EventArgs e)
        {
            string ExamID =  mcbExamRV.SelectedValue.ToString();
            LoadDatSource(ExamID);
        }

        private void LoadComboboxExam()
        {
            using (var qlttn = new QLThiTracNghiemDataContext())
            {
                ...
                var getExamId = from ler in listExamResult
                                ...
                                where ls.Username == CurrentUser.Username
                                select new { IdE = ex.ExamId, NameE = ex.ExamName };

                mcbExamRV.DataSource = getExamId.ToList();
                mcbExamRV.DisplayMember = "NameE";
                mcbExamRV.ValueMember = "IdE";
            }
        }

        private void LoadDatSource(string ExamId)
        {
            using (var qlttn = new QLThiTracNghiemDataContext())
            {
                var listTeach = qlttn.Teaches.ToList<Teach>();
                ...
                var result = from ler in listExamResult
                                join led in listExamDetail on ler.ExamDetailId equals led.ExamDetailId
                                join exc in qlttn.ExamCodes on ler.ExamCodeId equals exc.ExamCodeId
                                join g in qlttn.Subjects on exc.SubjectId equals g.SubjectId
                                join ex in qlttn.Exams on led.ExamId equals ex.ExamId
                                join ls in listStudent on ler.StudentId equals ls.StudentId
                                join u in qlttn.Users on ls.Username equals u.Username
                                where ls.Username == CurrentUser.Username
                                && led.ExamId == ExamId
                                select new {
                                    FullName = u.FirstName + " " + u.LastName,
                                    DOB  = u.Dob.Value.ToString("dd/MM/yyyy"),
                                    Phone  = u.Phone,
                                    ExamName =  ex.ExamName,
                                    SubjectName = g.SubjectName,
                                    NOQ = ler.NumberOfQuestionsAnswered,
                                    NOC = ler.NumberOfCorrectAnswers,
                                    Mark = ler.Mark
                                
                                };
                rpvTranscriptRS.LocalReport.DataSources.Clear();
                var examRSBS = new BindingSource();
                var examRPDS = new ReportDataSource("TranscriptRP");
                examRSBS.DataSource = result.ToList();
                examRPDS.Value = examRSBS;

                rpvTranscriptRS.LocalReport.DataSources.Add(examRPDS);
                rpvTranscriptRS.RefreshReport();
            }
        }
